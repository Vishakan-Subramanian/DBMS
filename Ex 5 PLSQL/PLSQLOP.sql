...
SQL> DECLARE
  ...
  6  
  7  	     CURSOR p_select IS
  8  		     SELECT *
  9  		     FROM pizza
 10  		     WHERE pizza_type = ptype;
 11  
 12  BEGIN
 13  	     ptype := '&ptype';
 14  
 15  	     OPEN p_select;
 16  
 17  	     FETCH p_select INTO p_id, pizza_type, u_price;
 18  
 19  	     IF p_select%ROWCOUNT = 0 THEN
 20  		     DBMS_OUTPUT.PUT_LINE('Pizza type: '||ptype||' is not available.');
 21  	     ELSE
 22  		     DBMS_OUTPUT.PUT_LINE('Pizza type: '||ptype||' is available.');
 23  	     END IF;
 24  
 25  	     CLOSE p_select;
 26  END;
...
new  16: 	c_name := 'Hari';
Enter value for start_date: 26-JUN-15
old  17: 	start_date := '&start_date';
new  17: 	start_date := '26-JUN-15';
